...
public:
    ...
    // Grana Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between granad, and grana-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<granaaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Grana Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/granapay/grana>";
    const std::string URL_WEBSITE = "<https://grana.org>";
...
/** Sanity checks
 *  Ensure that Grana Core is running in a usable environment with all
...
{
    // Make sure only a single Grana Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Grana Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Grana Core is started while in a temporary directory.\n",
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/granapay/grana/pull/1817 and https://github.com/granapay/grana/pull/1743
    ...
    // ********************************************************* Step 10c: schedule Grana-specific tasks
