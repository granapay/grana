// Copyright (c) 2018-2021 The Grana Core developers
...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The grana address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The grana address to use for payee updates and proposal voting.\n"
            "                              The corresponding private key does not have to be known by your wallet.\n"
            "                              The address must be unused and must differ from the collateralAddress.\n"
        },
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The grana address to use for masternode reward payments.\n"
        },
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Grana\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (!request.params[paramIdx + 6].isNull()) {
        ...
        if (!IsValidDestination(fundDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Grana address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (!request.params[5].isNull()) {
        ...
        if (!IsValidDestination(feeSource))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Grana address: ") + request.params[5].get_str());
    } else {
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (!request.params[4].isNull()) {
        ...
        if (!IsValidDestination(feeSourceDest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Grana address: ") + request.params[4].get_str());
        FundSpecialTx(pwallet, tx, ptx, feeSourceDest);
