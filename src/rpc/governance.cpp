...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 GRANA irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on granacentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in grana.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("grana.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in grana.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* Grana features */
    { "grana",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "grana",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "grana",               "gobject",                &gobject,                {} },
    { "grana",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
