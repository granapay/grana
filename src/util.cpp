...
const char * const BITCOIN_CONF_FILENAME = "grana.conf";
const char * const BITCOIN_PID_FILENAME = "granad.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "granad -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GranaCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GranaCore
    // Mac: ~/Library/Application Support/GranaCore
    // Unix: ~/.granacore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GranaCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GranaCore";
#else
    // Unix
    return pathRet / ".granacore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty grana.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Grana Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Grana Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2014, nEndYear) + "The Grana Core developers";
    }
