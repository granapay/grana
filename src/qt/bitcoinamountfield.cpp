...
public:
    explicit AmountValidator(QObject *parent) :
        QValidator(parent),
        currentUnit(BitcoinUnits::GRANA) {}
...
public:
    ...
    QSize minimumSizeHint() const
    {
        ...
        int h = 0;
        int w = fm.width(BitcoinUnits::format(BitcoinUnits::GRANA, BitcoinUnits::maxMoney(), false, BitcoinUnits::separatorAlways));
        w += 2; // cursor blinking space
        w += GUIUtil::granaThemeActive() ? 24 : 0; // counteract padding from css
        return QSize(w, h);
    }
