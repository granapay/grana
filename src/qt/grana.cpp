...
{
    return QCoreApplication::translate("grana-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in grana.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in grana.qrc)
...
/** Class encapsulating Grana Core startup and shutdown.
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // grana: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Grana Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Grana Core - \""), false, OptionsCategory::GUI);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(grana);
    Q_INIT_RESOURCE(grana_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse grana.conf
    ...
    // grana: links repeatedly have their payment requests routed to this process:
