...
*Setup instructions for a Gitian build of Grana Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Grana
...
- [Building Grana Core](#building-grana-core)
...
Clone the git repositories for Grana Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/granapay/grana
...
in the Grana Core repository under 'Fetch and create inputs' to install sources which require
...
To build Grana Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the Grana Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/grana/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/granapay/grana
...
For example:
```bash
URL=https://github.com/crowning-/grana.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit grana=${COMMIT} --url grana=${URL} ../grana/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit grana=${COMMIT} --url grana=${URL} ../grana/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit grana=${COMMIT} --url grana=${URL} ../grana/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the grana git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../grana/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/grana
SIGPATH=/some/root/path/grana-detached-sigs

./bin/gbuild --url grana=${BTCPATH},signature=${SIGPATH} ../grana/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/grana-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/grana-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/grana-osx-build.assert
```
...
[grana/gitian.sigs](https://github.com/granapay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
