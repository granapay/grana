...
- [Development guidelines](#development-guidelines)
    - [General Grana Core](#general-grana-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Grana Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/granapay/grana/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_grana
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_grana --log_level=test_suite
$ valgrind -v --leak-check=full src/granad -printtoconsole
```
...
# A coverage report will now be accessible at `./test_grana.coverage/index.html`.
```

**Sanitizers**

Grana Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Grana Core, where everyone uses
...
pay attention to for reviewers of Grana Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Grana Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Grana Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `grana-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `grana-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Grana Core, the wallet was always
