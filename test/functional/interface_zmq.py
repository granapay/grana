...
from test_framework.mininode import granahash
from test_framework.util import (assert_equal,
                                 bytes_to_hex_str,
                                 hash256,
                                 )

def granahash_helper(b):
    return encode(granahash(b)[::-1], 'hex_codec').decode('ascii')
...
class ZMQTest (BitcoinTestFramework):
    ...
    def _zmq_test(self):
        ...
        for x in range(num_blocks):
            ...
            assert_equal(genhashes[x], granahash_helper(block[:80]))
