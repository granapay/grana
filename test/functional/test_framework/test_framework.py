...
class BitcoinTestFramework():
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true",
                          help="Leave granads and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true",
                          help="Don't stop granads after the test execution")
        parser.add_option("--srcdir", dest="srcdir", default=os.path.abspath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"),
                          help="Source directory containing granad/grana-cli (default: %default)")
        ...
        parser.add_option("--usecli", dest="usecli", default=False, action="store_true",
                          help="use grana-cli instead of RPC for all commands")
        parser.add_option("--granad-arg", dest="granad_extra_args", default=[], type='string', action='append',
                          help="Pass extra args to all granad instances")
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/granad' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/grana-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.granad_extra_args
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: granads were not stopped and may still be running")
    ...
    def stop_node(self, i, wait=0):
        """Stop a granad test node"""
        self.nodes[i].stop_node(wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple granad test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as granad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self, extra_args=None, stderr=None):
        ...
        if create_cache:
            ...
            # Create cache directories, run granads:
            self.set_genesis_mocktime()
            for i in range(MAX_NODES):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in grana.conf
...
class GranaTestFramework(BitcoinTestFramework):
    def set_grana_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False):
        ...
        self.set_grana_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in grana-q-recovery thread
    ...
    def set_grana_dip8_activation(self, activate_after_block):
    ...
    def set_grana_llmq_test_params(self, llmq_size, llmq_threshold):
...
def skip_if_no_bitcoind_zmq(test_instance):
    """Skip the running test if granad has not been compiled with zmq support."""
    config = configparser.ConfigParser()
    config.read_file(open(test_instance.options.configfile))

    if not config["components"].getboolean("ENABLE_ZMQ"):
        raise SkipTest("granad has not been built with zmq enabled.")
