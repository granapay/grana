...
def granahash(s):
    return grana_hash.getPoWHash(s)
...
# Objects that map to granad objects, which can be serialized/deserialized
...
class CBlockHeader():
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(granahash(r))
            self.hash = encode(granahash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in granad indicates these should be deserialized as blocks
